...
## הספרייה נשלחת עם האתר

`public/content/articles.json` מכיל את הספרייה המלאה ונשלח כחלק מהבנייה, כך שהמאמרים על המסך ברגע שהדף על המסך — בלי מפתח, בלי בקשת רשת ובלי מרוץ. `scripts/export-library.mjs` מייצא אותו ממסד הנתונים, ורץ אוטומטית בכל פריסה. אם הייצוא נכשל, העותק שכבר בריפו תקף וממשיך לשרת.

מסד הנתונים נשאר השכבה החיה: מאמר שנוצר אחרי הפריסה האחרונה מגיע ממנו.
